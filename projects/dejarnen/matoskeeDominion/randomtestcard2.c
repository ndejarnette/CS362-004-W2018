...
void villageTestOracle(struct gameState *state, int prevHandCount, int prevActions, int prevDeck, int handPos) {
...
int main()
{
	...
	int failures = 0;
	int hCount, cardPosition, dCount, dkCount, actions;
	...
	for(i = 0; i < TEST_RANGE; i++) {
		...
		// refactored for teammate testing
		cardVillage(&testG, cardPosition);

		villageTestOracle(&testG, hCount, actions, dkCount, cardPosition);
	}
